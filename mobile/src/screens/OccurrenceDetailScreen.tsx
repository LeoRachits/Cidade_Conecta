...
const CATEGORY_LABELS: Record<string, string> = { ROAD:'🛣️ Via/Buraco', LIGHTING:'💡 Iluminação', GARBAGE:'🗑️ Lixo', FLOODING:'🌊 Alagamento', OTHER:'📌 Outro' }
...
export default function OccurrenceDetailScreen({ route }: any) {
  ...
  const API_BASE = process.env.EXPO_PUBLIC_API_URL?.replace('/api','') ?? 'http://192.168.1.100:3333'
    ...
    <ScrollView style={{ backgroundColor:'#F4F6FA' }} contentContainerStyle={{ padding:20, paddingBottom:40 }}>
      ...
      {data.photoUrl && (
        <View style={s.card}>
          ...
          <Image source={{ uri: `${API_BASE}${data.photoUrl}` }} style={s.photo} resizeMode="cover" />
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  infoLbl: { fontSize:12, color:'#888', width:80 },
